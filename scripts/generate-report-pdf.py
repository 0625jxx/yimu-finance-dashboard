...
CLASS_NAME = os.environ.get("YIMU_CLASS_NAME", "").strip()
INSTRUCTOR = os.environ.get("YIMU_INSTRUCTOR", "").strip()
if not CLASS_NAME or not INSTRUCTOR:
    raise RuntimeError("生成提交材料前必须设置 YIMU_CLASS_NAME 和 YIMU_INSTRUCTOR")
...
story = [
    ...
    Spacer(1, 1.0 * cm),
    data_table(["项目", "信息"], [
        ["学院", "计算机与信息工程学院"], ["小组成员", "蓝思钰 52302042011；姜萱萱 52302042008；刘晓盼 52302042019；王晨 52302042035；朱婷婷 52302042055"],
        ["班级", CLASS_NAME], ["指导教师", INSTRUCTOR], ["完成日期", "2026 年 9 月"]
    ], [4.2 * cm, 9.5 * cm]),
    PageBreak(),
    H1("摘  要"),
    ...
    P("系统采用 MVC 分层架构和观察者模式。Controller 订阅 Model 的具名状态事件，View 根据只读快照渲染响应式界面。Java 17 本地服务通过 REST API 将账户、交易、预算、目标和导入批次规范化写入 SQLite。项目提供 39 项 Node 自动化测试、5 项 JUnit 集成测试、真实浏览器回归脚本、可执行胖 JAR 和一键启动脚本。"),
    ...
    *bullets(["易用：桌面和移动端均可操作；金额可一键隐藏。", "正确：整数分计算；转账不计入收支；账户引用完整。", "安全：输出编码；CSV 限制；服务器限制方法、路径和内容类型。", "可维护：Model、View、Controller 职责分离；计算函数可独立测试。", "可部署：同时提供 Java 17 JAR、Node 开发服务器和一键运行脚本。"]),
    ...
    P("表现层由响应式 HTML/CSS 与 DashboardView 组成；控制层由 AppController 处理导航、表单和业务编排；领域层由 FinanceStore 与纯计算函数组成；持久化适配器在 JAR 模式下调用 REST API 和 SQLite，在 Node 开发模式下回退 localStorage。"),
    ...
    data_table(["层次", "主要文件", "职责"], [
        ...
        ["持久化层", "persistence.js / SqliteFinanceRepository", "REST 适配与 SQLite 事务"]
    ], [2.5 * cm, 5.0 * cm, 7.2 * cm]),
    PageBreak(), H1("4 详细设计"), H2("4.1 核心数据模型"),
    figure(ASSETS / "dataModel.png", 16.0 * cm, 8.0 * cm, "图 4-1  SQLite 数据库 ER 图"),
    data_table(["数据表", "主键", "说明"], [
        ["accounts", "account_id", "账户与期初余额"],
        ["transactions", "transaction_id", "收入、支出和转账事实"],
        ["budgets", "budget_month + category", "分类月度预算"],
        ["goals", "goal_id", "储蓄或还债目标"],
        ["import_batches", "batch_id", "批次可追溯和整批撤销"],
        ["app_meta", "meta_key", "数据库初始化状态"]
    ...
    *bullets(["资产余额 = 期初余额 + 收入 + 转入 - 支出 - 转出。", "负债余额 = 期初未偿金额 + 消费支出 - 还款转入。", "净资产 = 资产余额合计 - 负债余额合计。", "结余率 = (收入 - 支出) / 收入；收入为零时显示不适用。", "预算使用率低于 80% 为正常，达到 80% 为提醒，达到 100% 为超支。", "转账只改变相关账户余额，不进入收入、支出和净现金流统计。"]),
    ...
    H2("6.3 数据导入与部署"),
    P("CSV 解析器支持中文表头和带引号字段，将元转换为整数分。每次导入形成独立批次并写入关联交易，撤销时只删除该批次。Maven 将 Web 资源、SQLite JDBC 和 Jackson 打入胖 JAR；本地服务器仅绑定 127.0.0.1:4173，并提供同源状态 API。"),
    PageBreak(), H1("7 测试与质量保证"), H2("7.1 测试层次"),
    data_table(["层次", "覆盖内容", "结果"], [
        ["Node 单元测试", "余额、净资产、预算、CSV、观察者、目标、报表", "通过"],
        ["JUnit 集成测试", "建表、外键、事务、HTTP API、跨重启恢复", "通过"],
        ["结构与安全", "MVC、页面、路径穿越、隐藏目录、JAR 配置", "通过"],
        ["浏览器回归", "编辑、导入撤销、预算、目标、报表、响应式", "通过"]
    ...
    P("截至提交版本，Node 自动化测试共 39 项、JUnit 集成测试共 5 项，全部通过；真实 Chromium 回归在交付 JAR 上执行通过，页面无控制台错误。"),
    H1("8 安全性与可靠性"),
    *bullets(["用户字段在进入 innerHTML 前统一做 HTML 实体编码。", "CSV 与状态请求体上限为 2MB；金额必须为正；账户引用必须存在。", "静态资源只允许 GET/HEAD，状态 API 只允许 GET/PUT。", "SQLite 启用外键并在单一事务内更新规范化表。", "响应包含 CSP、nosniff、禁止嵌入和来源策略。", "产品化时仍需补充认证、授权、加密与备份。"]),
    PageBreak(), H1("9 项目总结"),
